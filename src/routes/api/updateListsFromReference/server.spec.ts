...
vi.mock('$lib/clients/pihole_client', () => ({
	getListsFromReference: vi.fn(),
	updateListForInstance: vi.fn()
}));
...
describe('POST /api/updateListsFromReference', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	});

	it('delegates to syncItemsFromReference with the lists getter and updater', async () => {
		mockedSync.mockResolvedValue(new Response(null, { status: 200 }));

		await POST({} as never);

		expect(mockedSync).toHaveBeenCalledOnce();
		expect(mockedSync).toHaveBeenCalledWith('lists', getListsFromReference, updateListForInstance);
	});
