...
const mockedResume = vi.mocked(resumeDNSBlocking);
...
describe('POST /api/resumeDNSBlocking', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		mockedResume.mockResolvedValue(true);
	});
	...
	it('returns success when resumeDNSBlocking succeeds', async () => {
	...
	it('calls resumeDNSBlocking with the matched instance', async () => {
		const instance = makeInstance();
		mockedGetInstances.mockResolvedValue([instance]);

		await POST(makeRequest({ instanceId: 1 }) as never);

		expect(mockedResume).toHaveBeenCalledWith(instance);
	});
});
